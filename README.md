...
Human‑readable, cross‑chain naming for the Nominal Resolution Protocol. This Solana program is the minimal on‑chain registry that maps a name (string) to its owner, resolved address, optional token fee configuration, and a per‑user primary name record. Complex cross‑chain logic, authority scoring, and propagation live in the off‑chain / client SDK (see litepaper).
